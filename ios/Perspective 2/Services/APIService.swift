...
class APIService: ObservableObject {
    ...
    // Overload without request body (avoids generic-parameter inference issues)
    private func makeRequest<U: Codable>(
        endpoint: String,
        method: String,
        responseType: U.Type
    ) -> AnyPublisher<U, APIError> {
        // Call through with body == nil and a dummy type
        return makeRequest(endpoint: endpoint, method: method, body: Optional<Data>.none as Data?, responseType: responseType)
            .eraseToAnyPublisher()
    }
    ...
    private func makeAuthenticatedRequest<U: Codable>(
        ...
        return makeAuthenticatedRequest(endpoint: endpoint, method: method, body: Optional<Data>.none as Data?, responseType: responseType)
            .eraseToAnyPublisher()
    }
}
