import { ChallengeResponse, ChallengeOption, ChallengeContent } from '../types/api-contracts';
...
export class ChallengeTransformService {
   ...
   */
  private static parseAndNormalizeContent(rawContent: any): { content: ChallengeContent; options: ChallengeOption[] | null } {
    let content: any = {};
    ...
    return {
      ...
      streakInfo: {
        ...
        isActive: this.parseBoolean(
          result.streakInfo?.streakMaintained || 
          result.streakInfo?.isActive || 
          result.streakInfo?.is_active, 
          false
        )
      }
    };
   ...
   */
  static transformChallengeStatsForAPI(stats: any): any {
