  ...
  res.json = function(body: any) {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(`✅ Response validation passed for ${endpoint}`);
      }
    } catch (error) {
...
function validateChallengeEndpoint(body: any, type: string) {
  ...
  if (!body.type || !isValidChallengeType(body.type)) errors.push('type must be a valid challenge type');
  ...
  if (body.options !== null && body.options !== undefined) {
    if (!Array.isArray(body.options)) {
      ...
      body.options.forEach((opt: any, i: number) => {
        if (typeof opt.id !== 'string') errors.push(`options[${i}].id must be a string`);
        if (typeof opt.text !== 'string') errors.push(`options[${i}].text must be a string`);
        if (typeof opt.isCorrect !== 'boolean') errors.push(`options[${i}].isCorrect must be a boolean`);
      });
    }
  }
...
function validateChallengeStatsEndpoint(body: any) {
  ...
  if (typeof body.totalCompleted !== 'number') errors.push('totalCompleted must be a number');
  if (typeof body.currentStreak !== 'number') errors.push('currentStreak must be a number');
  if (typeof body.longestStreak !== 'number') errors.push('longestStreak must be a number');
  if (typeof body.averageAccuracy !== 'number') errors.push('averageAccuracy must be a number');
  if (typeof body.totalXpEarned !== 'number') errors.push('totalXpEarned must be a number');
  if (typeof body.challengesByType !== 'object') errors.push('challengesByType must be an object');
  if (!Array.isArray(body.recentActivity)) {
    errors.push('recentActivity must be an array');
  } else if (body.recentActivity.length === 0) {
    console.warn('⚠️ recentActivity is empty - iOS expects ChallengeActivity objects');
  }
...
function validateLeaderboardEndpoint(body: any) {
  ...
  body.forEach((entry: any, i: number) => {
    ...
    if (typeof entry.id !== 'number') errors.push(`[${i}].id must be a number`);
    if (typeof entry.username !== 'string') errors.push(`[${i}].username must be a string`);
    if (typeof entry.challengesCompleted !== 'number') errors.push(`[${i}].challengesCompleted must be a number`);
    if (typeof entry.totalXp !== 'number') errors.push(`[${i}].totalXp must be a number`);
    if (typeof entry.correctAnswers !== 'number') errors.push(`[${i}].correctAnswers must be a number`);
    
    if (entry.avatarUrl === null) {
      console.warn(`⚠️ Leaderboard entry[${i}].avatarUrl is null - TODO: implement avatars`);
    }
...
function validateChallengeSubmitEndpoint(body: any) {
  ...
  if (typeof body.isCorrect !== 'boolean') errors.push('isCorrect must be a boolean');
  if (typeof body.feedback !== 'string') errors.push('feedback must be a string');
  if (typeof body.xpEarned !== 'number') errors.push('xpEarned must be a number');
  
  if (!body.streakInfo || typeof body.streakInfo !== 'object') {
    errors.push('streakInfo must be an object');
  } else {
    if (typeof body.streakInfo.current !== 'number') errors.push('streakInfo.current must be a number');
    if (typeof body.streakInfo.longest !== 'number') errors.push('streakInfo.longest must be a number');
    if (typeof body.streakInfo.isActive !== 'boolean') errors.push('streakInfo.isActive must be a boolean');
  }
...
function validateAuthEndpoint(body: any) {
  ...
  if (typeof body.token !== 'string') errors.push('token must be a string');
...
function validateUserObject(user: any, errors: string[]) {
  ...
  if (typeof user.username !== 'string') errors.push('user.username must be a string');
  if (typeof user.echoScore !== 'number') errors.push('user.echoScore must be a number, not string');
  if (typeof user.currentStreak !== 'number') errors.push('user.currentStreak must be a number');
